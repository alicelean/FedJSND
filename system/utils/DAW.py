...
#
#
#     def update_aggregation_weight(self,
...
#
#         # initialize the weight to all ones in the beginning,w0(初始值)
#         if self.weights == None:
#             self.weights = [torch.ones_like(param.data).to(self.device) for param in params_p]
#         for param_t,weight in zip(params_t, self.weights):
#             weight.data = torch.clamp(weight *self.w0 , 0, 1)
#
#
...
#             for param_c, param_g in zip(client.params, params_g, self.weights):
#                 s = client.sizerate * (param_c + 1 * param_g) * param_t.grad
#         for client in self.clients:
#             for param_c, param_g in zip(client.params, params_g, self.weights):
#                 s += client.sizerate * (param_c + 1 * param_g) * param_t.grad
#         weight.data = torch.clamp(weight - self.eta * s, 0, 1)
#
#         #求提度
#         while True:
